...
[Source code](https://github.com/hicks017/MLB-rule-changes "GitHu: Hicks017 MLB-rule-changes"){target="_blank"} \| PDF version (WIP)
...
All data-retrieval and preprocessing was executed using [Python 3.11.6](https://www.python.org/downloads/release/python-3116/ "Python Release 3.11.6"){target="_blank"}. The exploration and final analysis were written with [R 4.4.3](https://cran.r-project.org/bin/windows/base/old/4.4.3/ "CRAN - R 4.4.3"){target="_blank"} within a lightweight Docker container. Versioned scripts and written reports were saved on a public [GitHub repository](https://github.com/hicks017/MLB-rule-changes "MLB Rule Changes - Christian Hicks"){target="_blank"} to ensure transparency.

Raw data originated from the free, official [MLB Stats API.](https://statsapi.mlb.com/api/ "MLB Stats API base URL"){target="_blank"} No personal player data or personally identifiable information were used.
...
# t test
ttest_total_duration <- t.test(duration_all_plays ~ rule_change, data = tbl_game_duration)
```
...
# Environemnt variables for pitch clock t tests
ttest_total_duration_estimate <- unname(round(
  (ttest_total_duration[["estimate"]][1] - ttest_total_duration[["estimate"]][2]) * 60,
  1
))

ttest_total_duration_ci <- sprintf(
  '%.1f to %.1f',
  ttest_total_duration[['conf.int']][1] * 60,
  ttest_total_duration[['conf.int']][2]* 60
)
```

Individual play durations were summed to determine a combined duration of game play. An overall reduction was observed after pitch clock implementation (@fig-total-duration). Mean combined play duration decreased by `r ttest_total_duration_estimate` minutes (95% CI: `r ttest_total_duration_ci`. @tbl-total-duration).

```{r}
#| code-summary: Plot
#| fig-cap: Bar chart of mean total play duration (hours) from years 2014-2024.
...
```{r}
#| code-summary: Table
#| tbl-cap: Mean total play duration (hours) by year across 25,192 games.
#| label: tbl-total-duration

tbl_game_duration |>
  select(game_pk, season, duration_all_plays) |>
  pivot_wider(
    id_cols = game_pk,
    names_from = season,
    values_from = duration_all_plays
  ) |>
  gtsummary::tbl_summary(
    include = -game_pk,
    type = everything() ~ "continuous",
...
# Get OBP for NL pitcher and DH positions
df_nl_ops <- read_csv(file.path(path, "nl_ops.csv"))

# Remove duplicates
df_nl_ops <- distinct(df_nl_ops)

# Factorize season
df_nl_ops <- mutate(df_nl_ops, year = factor(year))
```

TEXT (batting appearances, with discussion showing that pitchers had fewer anyways and required additional management).
...
```{r}
#| code-summary: Plot
#| fig-cap: Bar chart of mean on-base percentage (OBP) by position from years 2014-2024.
#| label: fig-obp
...
df_nl_ops |>
  ...
  summarise(
    obp_mean = mean(OBP, na.rm = TRUE),
    obp_sd = sd(OBP, na.rm = TRUE),
    .groups = "drop"
  ) |>
  
  # Plot
  ggplot(aes(x = year, y = obp_mean, fill = position)) +
  geom_errorbar(
    aes(ymin = obp_mean, ymax = obp_mean + obp_sd),
    width = 0.2,
    color = "gray10",
    alpha = 0.75
  ) +
  ...
  scale_fill_manual(
    values = c(DH = dodger_red, P = dodger_blue)
  ) +
  theme(axis.title.y = element_text(angle = 0, vjust = 0.5)) +
  labs(
    title = "Mean on-base percentage by position and year",
    x = "Season",
    y = "OBP",
    fill = "Position"
  )
...
```{r}
#| code-summary: Table
#| tbl-cap: Mean mean on-base percentage (OBP) of P/DH position by year across 12,602 National League home games.
#| label: tbl-obp

df_nl_ops |>
  select(game_pk, year, OBP) |>
  pivot_wider(
    id_cols = game_pk,
    names_from = year,
    values_from = OBP
...
pauses <- 
  ...
  mutate(diff = paste0(round((`3-batter minimum` - `No minimum`) * 100), "%")) |>
  select(name, diff) |>
  tibble::deframe()
```
