...
df_steals |>
  ...
  tidyr::pivot_wider(id_cols = team_id, names_from = season, values_from = attempts) |>
...
df_steals |>
  ...
  tidyr::pivot_wider(id_cols = team_id, names_from = season, values_from = sb_success_rate) |>
  gtsummary::tbl_summary(
    include = -team_id,
    missing = "no",
    statistic = everything() ~ "{mean} ± {sd}"
...
```{r}
#| code-summary: Table
#| tbl-cap: Mean pitcher appearances by year across 25,193 games.
#| label: tbl-pitchers
...
```{r}
#| code-summary: Table
#| tbl-cap: Mean mid-inning pitching changes by year across 25,193 games.
...
```{r}
#| code-summary: Table
#| tbl-cap: Count and proportions of  25,193 games with various numbers of pauses in play due to mid-inning pitching changes.
#| label: tbl-pauses
