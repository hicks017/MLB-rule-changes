...
ggplot(tbl_n_per_game_yearly, aes(x = year, y = n, fill = rule_change)) +
  ...
  scale_fill_manual(
    values = c("FALSE" = "#EF3E42", "TRUE" = "#005A9C"),
...
A mid-inning pitching change would add a pause in play that was not already scheduled, such as pauses in between innings. The differences in proportions of games with pauses in play due to mid-inning pitching changes were `r pauses[["pause_01"]]` for games with any added pauses, `r pauses[["pause_02"]]` for games with at least 2 pauses, and `r pauses[["pause_03"]]` for games with at least 3 pauses (See @tbl-pauses).
